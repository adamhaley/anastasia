...
{
    public $props;
    public function keywords($k = '', $name = '')
    {
        for($i = 0; $i < count($k); $i++) {
    ...
    public function generate_checkboxes($userwords = '')
    {
        $d = new display();
    ...
    public function checked2array($http_vars)
    {
    ...
    public function generate_dropdown($selected = '', $caption='', $onchange='')
    {
    ...
    public function generate_radios($selected)
    {
        while(list($key, $value) = each($this->props)) {
            if(($key != 'name') && $key != '') {
