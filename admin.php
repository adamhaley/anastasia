...
class admin
{
    ...
    public function admin($props=array())
    {
    ...
    public function start()
    {
        ...
        if($this->get_vars[p]) {
            $p = $this->get_vars[p];
        } elseif($this->post_vars[p]) {
            $p = $this->post_vars[p];
        } elseif($what != $this->session_vars[what]) {
            $p = 1;
        } elseif($this->session_vars[p]) {
            $p = $this->session_vars[p];
        }

        if($what != $this->session_vars[what]) {
            $sortby = 'id';
        } elseif($this->get_vars[sortby]) {
            $sortby = $this->get_vars[sortby];
        } elseif($this->post_vars[sortby]) {
    ...
    public function body()
    {
        ...
        } elseif($post_vars[action]) {
            $action = $post_vars[action];
        }
        //get what
        if($get_vars[what]) {
            $what = $get_vars[what];
        } elseif($post_vars[what]) {
            $what = $post_vars[what];
        }
        //get id
        if($get_vars[id]) {
    ...
    public function edit_element($what, $id='')
    {
        $h = $this->html;
        $post_vars = $this->post_vars;
        $post_files = $this->post_files;
...
<input type="hidden" name="id" value="' . $id . '">';
    ...
    public function manage($what)
    {
        ...
        if($this->get_vars[sortby]) {
            ...
            $sortby = $this->post_vars[sortby];
        } elseif($this->session_vars[sortby]) {
...
*/
    public function view_element($what, $id)
    {
        if($this->bps[$what]) {
            $bp = $this->bps[$what];
        } else {
...
r>";
    ...
    public function duplicate_element($what, $id)
    {
        $bp = $this->bps[$what];
        $e = new element($bp, $id);
        $newe = new element($bp);
    ...
    public function archive_element($what, $id)
    {
        $bp = $this->bps[$what];
        $e = new element($bp, $id);
    ...
    public function wrap_in_template($admin, $tid)
    {
