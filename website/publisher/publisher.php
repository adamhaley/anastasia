...
class publisher
{
    public function publisher($frontend='')
    {
    ...
    public function replace_links_static($content)
    {
        preg_match_all("|<%link_(.*?)%>|i", $content, $arrayoflinks);
        while(list(, $link) = each($arrayoflinks[1])) {
            $filename = $link . ".html";
    ...
    public function replace_links_dynamic($content, $file='')
    {
        $l = new local();
    ...
    public function publish_additional_files()
    {
    ...
    public function frontend_request($get_vars_array)
    {
        $post_vars = array();
        $f=$this->frontend;
        $f->dynstatic = 'static';
